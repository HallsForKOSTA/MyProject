...
<head>
    ...
    <link rel="stylesheet" href="${ctx }/style/css/style.css"> <!-- 이렇게 해도 되는데 안함 contextpath 사용 -->
    
    <script type="text/javascript">
    // 로그인시 아이디, 패스워드를 입력했는지 확인
    // 입력하지 않은 항목에 대한 사용자 알림.
    
    
    	function loginValidate() {
			var loginId = document.getElementById("iptLoginId").value;
			var loginPw = document.getElementById("iptLoginPw").value;
			...
			if(loginId ===""){
				...
				alert("패스워드를 입력하세요");
				document.getElementById("iptLoginPw").focus();
				return false;
			}
			// 패스워드
...
<div class="login-wrap">
    ...
    <form action="${ctx}/login.do" method="post" onsubmit="return loginValidate();">
        ...
        <label>비밀번호</label>
        <input type="password" id = "iptLoginPw" name="passwd" placeholder="패스워드를 입력하세요.">
        <div class="login-btn">
